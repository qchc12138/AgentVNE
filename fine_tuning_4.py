...
def run_ppo_episode(
    ...
    # 标记序列结束
    if len(traj_done) > 0:
        traj_done[-1] = True  # 标记最后一个 workflow 为序列结束
    if len(env.traj) > 0:
        env.traj[-1]['done'] = True
    
    # 计算最终回报（基于整个 episode 的 r_t）
    ...
    if update_after_episode and len(traj_logp) > 0:
        ...
        rewards = torch.stack(traj_rew)        # [num_workflows]
...
def run_ppo_batch_training(
    ...
    for update_idx in range(num_updates):
        ...
        # ========== 批量PPO更新（基于合并的 Workflow 序列）==========
        # 重要：合并多个 episode 的 workflow 序列后进行更新
        # - 总样本数 = 所有 episode 中到达的 workflow 总数
        # - 每个样本对应一个 workflow 的处理过程
        if len(all_logprobs) > 0:
            logprobs_old = torch.cat(all_logprobs, dim=0)  # [total_workflows]
            values_old = torch.cat(all_values, dim=0)      # [total_workflows]
            rewards = torch.cat(all_rewards, dim=0)        # [total_workflows]
            
            print(f"\n  【PPO更新 {update_idx + 1}/{num_updates}】合并 {num_episodes_per_update} 个episode的workflow序列...")
            print(f"    总 workflow 数: {len(all_vn_list)}, 总奖励均值: {rewards.mean().item():.3f}")
            print(f"    开始PPO更新 (共{train_iters}次迭代):")
            agent.update(
                vn_list=all_vn_list,      # 合并的 workflow VN 列表
                sn_list=all_sn_list,      # 合并的 SN 状态列表
                mappings=all_mappings,    # 合并的映射列表
                logprobs_old=logprobs_old,
                values_old=values_old,
                rewards=rewards,
                dones=all_dones,          # 每个 episode 的最后一个 workflow 标记为 done
...
def get_model_paths(script_dir: str, use_finetuning_model: bool = False) -> Tuple[Optional[str], Optional[str]]:
    ...
    if use_finetuning_model:
        ...
        finetuning_policy_path = os.path.join(script_dir, 'finetuning_output_4', 'policy_network_latest.pth')
        finetuning_value_path = os.path.join(script_dir, 'finetuning_output_4', 'value_network_latest.pth')
