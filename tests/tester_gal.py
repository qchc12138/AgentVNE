...
__all__ = [
    ...
    "run_gal_strategy_test",
    "smoke_test_gal_strategy",
]


#region GAL 策略定义
class GALPlacementStrategy(PlacementStrategy):
    """包装 GAL 贪心策略以适配统一测试接口（不得直接修改主环境）。"""
...
def smoke_test_gal_strategy(*, detail_print: bool = False) -> Dict[str, any]:
    """较小参数的 GAL 冒烟测试，输出详细日志。"""
    ...
    printer = TestPrinter(
        ...
        test_scope="gal_single",
    )
    try:
        printer.start_round(
            table_title="GAL Strategy Smoke Test",
        ...
        printer.add_row(
            build_strategy_row("gal", result),
            strategy_info={"strategy_name": "GALPlacementStrategy"},
        )
        printer.finalize()
    finally:
        printer.close()
...
if __name__ == "__main__":
    summary = smoke_test_gal_strategy(detail_print=False)["summary"]
    print(
        "[GALSmoke] total_tasks={total}, accepted={accepted}, acceptance_rate={rate:.2f}%".format(
