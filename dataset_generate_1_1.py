...
def generate_pretrain_dataset(
    ...
    with tqdm(total=total_samples, desc="生成样本") as pbar:
        for episode_idx in range(num_episodes):
            ...
            is_first_episode = (episode_idx == 0)
            is_last_episode = (episode_idx == num_episodes - 1)

            if is_first_episode:
                print(f"\n=== 第一个 Episode {episode_idx + 1}/{num_episodes} ===")
            if is_last_episode:
            ...
            for wf_idx in range(workflows_per_episode):
                ...
                # 打印：第一个 episode 或测试 episode 或最后一个 episode 的放置前 NodeRank
                if is_first_episode or is_test_episode or is_last_episode:
