    ...
    def update(self,
        ...
        for iter_idx in range(train_iters):
            ...
            for vn, sn, mapping in zip(vn_list, sn_list, mappings):
                ...
                # 重建SN节点ID到索引的映射（与act函数中的逻辑一致）
                # probs_matrix的列索引对应sorted(env.G_sn.nodes())的顺序
                # 在act函数中：sn_node_list = sorted(env.G_sn.nodes())
                #              sn_id_to_idx = {sn_id: idx for idx, sn_id in enumerate(sn_node_list)}
                # 由于在update时没有env对象，我们需要从mapping中推断SN节点ID列表
                # 根据SN拓扑文件格式，SN节点ID通常是从0开始的连续整数
                # 因此，probs_matrix的列索引i对应SN节点ID=i（如果节点ID是连续的）
                if len(mapping) > 0:
                    # 从mapping中收集所有SN节点ID
                    all_sn_ids_in_mapping = sorted(set(mapping.values()))
                    # 检查SN节点ID是否在[0, N_s)范围内且连续
                    min_sn_id = min(all_sn_ids_in_mapping)
                    max_sn_id = max(all_sn_ids_in_mapping)
                    
                    # 如果SN节点ID在[0, N_s)范围内，假设ID就是索引（这是最常见的情况）
                    if min_sn_id >= 0 and max_sn_id < N_s:
                        # SN节点ID就是索引（从0开始连续）
                        sn_id_to_idx = {sn_id: sn_id for sn_id in all_sn_ids_in_mapping}
                    else:
                        # 如果SN节点ID不在[0, N_s)范围内，使用模运算（作为后备方案）
                        # 注意：这可能不准确，但如果没有环境信息，这是最好的猜测
                        # 这种情况应该很少见，因为SN节点ID通常是从0开始的连续整数
                        sn_id_to_idx = {sn_id: sn_id % N_s for sn_id in all_sn_ids_in_mapping}
                else:
                    sn_id_to_idx = {}
                ...
                for vn_idx, sn_id in mapping.items():
                    # 获取SN节点对应的索引
                    sn_idx = sn_id_to_idx.get(sn_id)
                    if sn_idx is None:
                        # 如果映射中没有，使用模运算作为后备（可能不准确）
                        sn_idx = sn_id % N_s
...
def save_training_results(training_stats: List[Dict], 
    ...
    if run_dir is not None:
        ...
        if output_dir is None:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            output_dir = os.path.join(script_dir, 'finetuning_output_3')
    ...
    # 3.5. 额外保存最新模型到 finetuning_output_3 目录（不带时间戳）
    ...
    # 6. 保存每个episode的平均r_t到CSV文件
    if all_episode_avg_rt:
        csv_path = os.path.join(run_dir, 'episode_avg_rt.csv')
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            # 写入表头
            writer.writerow(['Episode', 'Average_r_t'])
            # 写入数据：第一列是episode序号，第二列是平均r_t
            for idx, avg_rt in enumerate(all_episode_avg_rt, 1):
                writer.writerow([idx, f'{avg_rt:.6f}'])
        print(f"✓ Episode平均r_t CSV文件已保存: {csv_path}")
...
def get_model_paths(script_dir: str, use_finetuning_model: bool = False) -> Tuple[Optional[str], Optional[str]]:
    ...
    if use_finetuning_model:
        # 使用 finetuning_output_3 目录下的最新模型（微调后的模型）
        finetuning_policy_path = os.path.join(script_dir, 'finetuning_output_3', 'policy_network_latest.pth')
        finetuning_value_path = os.path.join(script_dir, 'finetuning_output_3', 'value_network_latest.pth')
...
if __name__ == '__main__':
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_base_dir = os.path.join(script_dir, 'finetuning_output_3')
    ...
    try:
        ...
        # ========== 模型选择配置 ==========
        # 设置为 True 使用预训练模型，False 使用随机初始化
        USE_PRETRAINED_MODEL = False  # 修改此参数来选择是否使用预训练模型
        
        # 如果使用预训练模型，设置为 True 使用微调后的最新模型，False 使用预训练模型（默认）
        USE_FINETUNING_MODEL = False  # 仅在 USE_PRETRAINED_MODEL=True 时生效
        ...
        if USE_PRETRAINED_MODEL:
            ...
            policy_ckpt_path, value_ckpt_path = None, None
            print(f"\n【模型配置】使用随机初始化的模型")
        
        training_stats, agent = run_ppo_batch_training(
            sn_topology_path=sn_path,
            workflow_types=workflow_types,
            policy_ckpt=policy_ckpt_path,  # None表示随机初始化，否则使用指定的预训练模型
            value_ckpt=value_ckpt_path,  # None表示随机初始化，否则使用指定的预训练价值网络
            device='cpu',
